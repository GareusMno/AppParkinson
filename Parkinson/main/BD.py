...
class Base(object):
    ...
    def sql_CreateTable(self):
        ...
        "IMC String, grasacorporal String, altura String, peso String, DNI String not null, gravedad String, medicacion String,estadio String);")
    ...
    def sql_InsertarPaciente(self,nombre,apellido,altura,peso,DNI,direccion,email,Telefono,SIP,nacimiento,genero,imc,grasacorporal,estadio):
        cursorObj = self.con.cursor()
        cursorObj.execute("INSERT INTO Pacientes(nombre,apellido,altura,peso,DNI,direccion,email,Telefono,SIP,nacimiento,genero,fechaingreso,IMC,grasacorporal,estadio) "+
        "VALUES('"+nombre+"','"+apellido+"','"+altura+"','"+peso+"','"+DNI+"','"+direccion+"','"+email+"','"+Telefono+"','"+SIP+"','"+nacimiento+"','"+genero+"',datetime('now','localtime'),'"+imc+"','"+grasacorporal+"','"+estadio+"');")
        self.con.commit()
    ...
    def sql_ActualizarPaciente(self,noriginal,nombre,apellido,SIP,medicacion,estadio):
        cursorObj = self.con.cursor()
        ...
        print(codigo[0])
        cursorObj.execute("UPDATE Pacientes SET nombre='"+nombre+"',apellido='"+apellido+"',SIP='"+SIP+"',medicacion='"+medicacion+"',estadio='"+estadio+"' where codigo="+str(codigo[0])+";")
        self.con.commit()
    #
    ...
    def sql_getDatosPacientes(self,nombre):
        cursorObj=self.con.cursor()
        cursorObj.execute(" SELECT nombre,apellido,SIP,nacimiento,estadio,medicacion FROM Pacientes where nombre='"+nombre+"'" )
        results=cursorObj.fetchall()
        Pacientes=[]
